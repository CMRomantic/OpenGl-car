...
#define RAW_MIC_CHAN_NUM          8
#define RAW_AEC_CHAN_NUM          2
#define RAW_FRAME_BYTES           (RAW_CHAN_NUM * 2)

#define SLOT_FRAMES               256                                // 16ms samples = 16 * 16 = 256
#define SLOT_FRAMES_BYTES         (SLOT_FRAMES * 2)                  // 1 channel(16bit) 1 slot bytes = 256 * 2
#define SLOT_FRAMES_PACKED_BYTES  (SLOT_FRAMES_BYTES * 2)
#define SLOT_FRAMES_ALL_RAW_BYTES (SLOT_FRAMES_BYTES * RAW_CHAN_NUM) // 10 channel 1 slot bytes
...
int uni_4mic_hal_init(int use4Mic)
{
    ...
    do{
        ...
        if(hal->use_4mic){
            ...
            uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_AEC_ON, &aec_on);
            uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_ENV_ON, &env_on);
            uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_FBF_ON, &fbf_on);
            // ������ǿ�Ƕȣ�Ĭ��ֵΪ�����ļ��еĳ�ʼ����EnhanceAngle
            uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_ENHANCE_ANGLE, &enhance_angle);
            // ������ǿ�Ƕȷ�Χ��Ĭ��ֵΪ�����ļ��еĳ�ʼ����BiasAngle
            uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_BIAS_ANGLE, &bias_angle);
            uni_hal_4mic_array_get_org(hal->MicHandle, UAL_FDE_PROTECT_TIME, &tmp);
            //	  printf("time :%f\n", tmp);
            uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_PROTECT_TIME, &time_protect);
            uni_hal_4mic_array_get_org(hal->MicHandle, UAL_FDE_PROTECT_TIME, &tmp);
            //	  printf("time :%f\n", tmp);

            uni_hal_4mic_array_get_org(hal->MicHandle, UAL_FDE_BIAS_ANGLE, &bias_angle);
	...
	while(hal->pcm_read_thread_running){
		if(pcm_is_ready(hal->pcm)){
            //	HAL_PRINT_ERR("[%s:%d] ERROR: pcm_read return %d. 1\n", __func__, __LINE__, ret);
                ...
                if(log_g_pcm==0){
                log_g_pcm = 1;
                HAL_PRINT_INFO("[%s:%d] first pcm_read \n", __func__, __LINE__);
            }
            ret = pcm_read(hal->pcm, pcm_buf, read_size);
            if(save_file_on==1){
                fwrite(pcm_buf, 1, read_size, file_all_read);
            ...
            if(0){
                if(g_write_count_raw==0){
                    ...
                    char filename[256];
                    sprintf(filename, "%s/file_raw_%d.pcm", "/data/misc/audioserver", g_debug_file_raw);
...
HAL_PRINT_DEBUG("\n");
			...
			if(g_flag_codec_id==1){
				if(frame_ids_v3[start_pos] != pcm_buf[1]){ 
					HAL_PRINT_ERR("[%s:%d] ERROR: the pcm v3 data is broken, [start_pos:%02d,%02x].\n", __func__, __LINE__, start_pos, pcm_buf[1]);			
					break;
				}
			} else {
...
static int unpack_raw (char *packbuf, int size, char *mic_pcm, char *aec_pcm)
{
    ...
    int sample_size = RAW_CHAN_NUM * 2;
#if 0
    if(save_file_on==1){
   ...
   // for(k = 0; k < size / PACKED_CHAN_NUM / 8; k++){
    for(k = 0; k < size / sample_size ; k ++){
        pos = packbuf + k * sample_size ;
    ...
    if(save_file_on==1) {
        fwrite(mic_pcm, 1, (size/sample_size) * 16, file_8mic);
        fflush(file_8mic); 
    }
...
#else
    if(save_file_on==1){
        fwrite(packbuf, 1, SLOT_FRAMES_BYTES * RAW_CHAN_NUM, file_all_read_org);
        fflush(file_all_read_org);
    }				
#endif
...
#define MICARRAY_OUTLEN_MAX         512
    ...
    do{
        if(1){
 ...
 #endif           
            mic_pcm_tmp = (char *)calloc(SLOT_FRAMES_ALL_RAW_BYTES / RAW_CHAN_NUM * RAW_MIC_CHAN_NUM / sizeof(short), sizeof(short));
            if(NULL == mic_pcm_tmp){
                //HAL_PRINT_ERR("%s: (NULL == mic_pcm)\n", __func__);
                break;
            }
            
            mic_pcm = (char *)calloc(SLOT_FRAMES_ALL_RAW_BYTES / RAW_CHAN_NUM * RAW_MIC_CHAN_NUM / sizeof(short), sizeof(short));
            if(NULL == mic_pcm){
                HAL_PRINT_ERR("%s: (NULL == mic_pcm)\n", __func__);
                break;
            }

            aec_pcm = (char *)calloc(SLOT_FRAMES_ALL_RAW_BYTES / RAW_CHAN_NUM * RAW_AEC_CHAN_NUM / sizeof(short), sizeof(short));
            if(NULL == aec_pcm){
                HAL_PRINT_ERR("%s: (NULL == aec_pcm)\n", __func__);
                break;
            }
            
            aec_pcm_tmp = (char *)calloc(SLOT_FRAMES_ALL_RAW_BYTES / RAW_CHAN_NUM * RAW_AEC_CHAN_NUM / sizeof(short), sizeof(short));
            if(NULL == aec_pcm_tmp){
                HAL_PRINT_ERR("%s: (NULL == aec_pcm)\n", __func__);
                break;
            }
            ...
            mic_pcm = (char *)calloc(SLOT_FRAMES_PACKED_BYTES / 2 * 4 / sizeof(short), sizeof(short));
            if(NULL == mic_pcm){
                HAL_PRINT_ERR("%s: (NULL == mic_pcm)\n", __func__);
                break;
            }

            aec_pcm = (char *)calloc(SLOT_FRAMES_PACKED_BYTES / 2 * 2 / sizeof(short), sizeof(short));
            if(NULL == aec_pcm){
                HAL_PRINT_ERR("%s: (NULL == aec_pcm)\n", __func__);
                break;
            }
        ...
        uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_DAMAGED_MIC_DETECT, &damaged_mic_detect);
        uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_DAMAGED_MIC_DETECTED_TIME, &(detected_time));


        //int idx = 1; 
        //uni_hal_4mic_array_set_org(hal->MicHandle, UAL_FDE_AEC_MORE, &idx);
        //set cpu
        int cpu_index = 0;
...
*/
            if(1){
                ...
                outlen = SLOT_FRAMES;
                p_out_data = (short *)out_pcm;
                Out_asr = (short *)mic_pcm;
	
                for (i = 0; i < outlen; i++){	
                    *(p_out_data + 2 * i) = *(Out_asr + i * RAW_MIC_CHAN_NUM);
                    *(p_out_data + 2 * i + 1) = *(Out_asr + i * RAW_MIC_CHAN_NUM); 
                }
                out_size = outlen * 4;
                if(g_pcm_read_error==0){
                    writeBuf(&hal->micArrayRingBuf, out_pcm, out_size);
            ...
            //  if(hal->debugMode){
           	if(0){
                if(g_write_count==0){
                  ...
                  char filename[256];
                  sprintf(filename, "%s/file_10ch_16000_16bit_%d.pcm", "/sdcard/unidata", g_debug_file);

                  char filenamenoflag[256];
                  sprintf(filenamenoflag, "%s/file_file_10ch_16000_16bit_%d.pcm", "/sdcard/unidata/", g_debug_file);
...
static int debug_record_files(void *handle, int on)
{
    ...
    if(on){
        ...
        sprintf(filename, "%s/file_raw_all.pcm", hal->env_cfg.debug_file_path);
        file_all_read = fopen(filename, "wb+");
        if(file_all_read == NULL) {
            HAL_PRINT_ERR("%s: %s open failed\n", __func__, filename);
        }

        memset(filename, 0, 512);
        sprintf(filename, "%s/file_aec.pcm", hal->env_cfg.debug_file_path);
        // file_aec_read = fopen(filename, "wb+");

        memset(filename, 0, 512);
        sprintf(filename, "%s/file_mic.pcm", hal->env_cfg.debug_file_path);
        //file_mic_read = fopen(filename, "wb+");

        memset(filename, 0, 512);
        sprintf(filename, "%s/file_out.pcm", hal->env_cfg.debug_file_path);
        file_out_read = fopen(filename, "wb+");

        memset(filename, 0, 512);
        sprintf(filename, "%s/file_raw_org.pcm", hal->env_cfg.debug_file_path);
        file_all_read_org = fopen(filename, "ab+");
        if(file_all_read_org == NULL) {
            HAL_PRINT_ERR("%s: %s open failed\n", __func__, filename);
        }


        memset(filename, 0, 512);
        sprintf(filename, "%s/file_8mic.pcm", hal->env_cfg.debug_file_path);
        file_8mic = fopen(filename, "ab+");
        if(file_8mic == NULL) {
            HAL_PRINT_ERR("%s: %s open failed\n", __func__, filename);
        }

        memset(filename, 0, 512);
        sprintf(filename, "%s/file_2aec.pcm", hal->env_cfg.debug_file_path);
        file_2aec_de = fopen(filename, "ab+");
        if(file_2aec_de == NULL) {
            HAL_PRINT_ERR("%s: %s open failed\n", __func__, filename);
        }

        memset(filename, 0, 512);
        sprintf(filename, "%s/file_out.pcm", hal->env_cfg.debug_file_path);
        file_out_read = fopen(filename, "ab+");
        if(file_out_read == NULL) {
            HAL_PRINT_ERR("%s: %s open failed\n", __func__, filename);
        }
        
        memset(filename, 0, 512);
        sprintf(filename, "%s/waking_8mic.wav", hal->env_cfg.debug_file_path);
        ...
        memset(filename, 0, 512);
        sprintf(filename, "%s/waking_2mic.wav", hal->env_cfg.debug_file_path);
        ...
        memset(filename, 0, 512);
        sprintf(filename, "%s/waking_out.wav", hal->env_cfg.debug_file_path);
        ...
        header_4mic.byte_rate = (header_4mic.bits_per_sample / 8) * header_4mic.num_channels * header_4mic.sample_rate;
        ...
        header_2aec.byte_rate = (header_2aec.bits_per_sample / 8) * header_2aec.num_channels * header_2aec.sample_rate;
        ...
        header_out.byte_rate = (header_out.bits_per_sample / 8) * header_out.num_channels * header_out.sample_rate;
        ...
        if(file_all_read_org) {
            fclose(file_all_read_org);
            file_all_read_org = NULL;
        }
        if(file_8mic == NULL) {
            fclose(file_2mic);
            file_8mic = NULL;
        ...
        if(file_aec_read) {
            fclose(file_aec_read);
            file_aec_read = NULL;
        }
        if(file_mic_read) {
            fclose(file_mic_read);
            file_mic_read = NULL;
        }
        if(file_out_read) {
            fclose(file_out_read);
...
void* uni_4mic_pcm_open(int ch_num)
{
    ...
    hal->ch_num = ch_num;
    //HAL_PRINT_ERR("zxl-ch_num=%d  tmp_flag %d  RAW_CHAN_NUM %d  RAW_SAMPLE_RATE %d\n", hal->ch_num, tmp_flag, RAW_CHAN_NUM, RAW_SAMPLE_RATE);
    printf("zxl-ch_num=%d  tmp_flag %d  RAW_CHAN_NUM %d  RAW_SAMPLE_RATE %d\n", hal->ch_num, tmp_flag, RAW_CHAN_NUM, RAW_SAMPLE_RATE);
    ...
    do{
       // card = hal->env_cfg.pcm_card;
        ...
        printf("zxl-card=%d, device=%d, i2s_bits=%d RAW_CHAN_NUM=%d RAW_SAMPLE_RATE = %d\n",
                card, device, i2s_bits, RAW_CHAN_NUM, RAW_SAMPLE_RATE);
        hal->pcm = pcm_open(card, device, PCM_IN, &pcm_config);
        if(NULL == hal->pcm || !pcm_is_ready(hal->pcm)) {
            printf("Unable to open PCM card=%d,device=%d, (%s)\n", card, device, pcm_get_error(hal->pcm));
            if(g_unisound_event_callback!=NULL){
        ...
        //if(hal->raw_pcm){
            // cache RECORD_BUF_CNT * 2 slot, 1 slot is 16ms
        ...
        //    min_rw_size = 4;
        //}
        HAL_PRINT_ERR("[%s] initRingBuf:MicArray, size:%d\n", __func__, ringbuf_size);
        ...
        ret = initRingBuf(&hal->micArrayRingBuf, "MicArray", ringbuf_size, 2);
        if(0 > ret){
        ...
        if(hal->env_cfg.high_priority){
            HAL_PRINT_ERR("[%s:%d] raw thread: set thread priority high\n", __func__, __LINE__);
            pattr = (void *)&attr;
        } else {
            pattr = NULL;
        }
        ...
        //if(hal->raw_pcm){
            ret = pthread_create(&hal->pcm_read_thread, pattr, pcm_read_raw_thread_proc, hal);
        //} else {
        //    ret = pthread_create(&hal->pcm_read_thread, pattr, pcm_read_packed_thread_proc, hal);
        //}
        if(ret){
        ...
        ret = pthread_create(&hal->mic_array_thread, pattr, mic_array_thread_proc, hal);
        if(ret){
